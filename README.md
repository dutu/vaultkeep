...
Vaultkeep is a backup application for Debian systems. It creates independent archive files from one or more files and directories, skips unchanged sources, applies calendar-based retention, and runs either manually or through managed systemd timers.
 ...
 > **Implementation status:** Milestones 1 through 5 are complete. The installable Python package, version command, configuration validation, source discovery, exclusions, deterministic full-content hashing, configuration and credential fingerprints, atomic local state, state reconstruction policy, unchanged detection, verified `.tar.zst` and password-protected `.tar.7z` creation, strict password handling, checksums, private plaintext cleanup, atomic finalization, template-derived backup names, immutable manifests, strict destination discovery, calendar-bucket retention planning, and guarded backup deletion are implemented. Backup commands, the installer, and systemd units are not implemented. Commands in this guide beyond `vaultkeep --version` describe the approved v1 user interface. Consult the capability table in [architecture_and_design.md](architecture_and_design.md#12-capability-status) for current implementation status.
