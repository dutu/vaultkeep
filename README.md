...
> **Implementation status:** Milestones 1 through 3 are complete. The installable Python package, version command, configuration validation, source discovery, exclusions, deterministic full-content hashing, configuration and credential fingerprints, atomic local state, state reconstruction policy, unchanged detection, dependency locks, tests, and continuous-integration quality gates are implemented. Archive creation, backup commands, destination-manifest discovery, the installer, and systemd units are not implemented. Commands in this guide beyond `vaultkeep --version` describe the approved v1 user interface. Consult the capability table in [architecture_and_design.md](architecture_and_design.md#12-capability-status) for current implementation status.
